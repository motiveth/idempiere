...
DROP VIEW IF EXISTS rv_transaction;
...
CREATE OR REPLACE VIEW rv_transaction AS 
 SELECT t.m_transaction_id,
    ...
    bp.isemployee,
    bp.issalesrep,
    COALESCE(io.C_Activity_ID, mov.C_Activity_ID, pro.C_Activity_ID, proplan.C_Activity_ID) AS C_Activity_ID
   FROM m_transaction t
     ...
     LEFT JOIN m_movement mov ON mov.m_movement_id = ml.m_movement_id
     ...
     LEFT JOIN m_production pro ON pro.m_production_id = prdl.m_production_id
     LEFT JOIN m_productionplan prdp ON prdl.m_productionplan_id = prdp.m_productionplan_id
     LEFT JOIN m_production proplan ON proplan.m_production_id = prdp.m_production_id
     ...
     LEFT JOIN c_bpartner bp ON bp.c_bpartner_id = COALESCE(mov.c_bpartner_id, io.c_bpartner_id, pro.c_bpartner_id, proplan.c_bpartner_id, pj.c_bpartner_id);

ALTER TABLE rv_transaction
  OWNER TO adempiere;

SELECT register_migration_script('201723061604_rv_transaction.sql') FROM dual
;
